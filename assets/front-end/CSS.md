...
1. 父标签末尾添加一个空标签，设置 clear: both;
2. 父标签添加伪元素，设置 clear: both;
3. 父元素添加 overflow: hidden;形成 BFC
4. `<br clear="all">`隔离浮动与非浮动标签
...
参考：[CSS3 新特性的概述](https://segmentfault.com/a/1190000010936764)

### 用 CSS 绘制三角形

display: block; width: 0; height: 0; border: 20px solid transparent; border-left: 10px solid #000;

### CSS 中的定位属性有哪些？区别是什么？

fixed: 固定定位, absolute: 绝对定位, relative: 相对定位, static: 静态定位.

### CSS 中隐藏元素的方式有哪些？区别是什么？
...
- **em：** 文本相对长度单位。相对于当前元素文本的字体尺寸。如果当前元素字体尺寸未被设置，则相对于继承的字体大小。
...
使用 flex 布局，通过设置子元素的 margin 实现自动居中
...
- Formatting context：格式化上下⽂，它是⻚⾯中的⼀块渲染区域，并且有⼀套渲染规则，它决定了其⼦元素将如何定位，以及和其他元素的关系和相互作⽤。
...
```html
.left{ width: 100px; height: 200px; background: red; float: left; } .right{
height: 300px; background: blue; overflow: hidden; }
...
可以先在 JS 中拿到 window.devicePixelRatio 的值，然后把这个值通过 JSX 或者模板语法给到 CSS 的 data 里
...
这里针对像素比为 2 的页面，把整个页面缩放为了原来的 1/2 大小。这样，本来占用 2 个物理像素的 1px 样式，现在占用的就是标准的一个物理像素。根据像素比的不同，这个缩放比例可以被计算为不同的值，用 js 代码实现
